...
class MergerTree(HasTraits):
    ...
    view = View(VGroup(Item(name='halopath',style='readonly'),
                 ...
                 Group(HGroup(Item(name='haloid',label='Halo ID',style='custom'),springy=True)),
                 ...
                 Group(HGroup(HGroup(Group(HGroup(VGroup(Group(Item(name='halo_varx',label='x-axis',springy=True),
                       ...
                       Group(Item(name='plot_button',show_label=False,springy=True),enabled_when='len(halo_type) > 0'),enabled_when='len(haloid) >= 1')))
    ...
    def _plot_button_fired(self):
        ...
        if len(self.haloid) >= 1:
            ...
            print "You need to select at least one halo."
